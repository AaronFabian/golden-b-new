...
class ChatRoomDaoImpl
{
   ...
   public function fetchMessageData(ChatRoom $chatRoom)
   {
      ...
      $query = "SELECT from_admin_nik, message, room_created_date,room_id, admin.name AS name_for_display, admin.images FROM chat_room JOIN admin ON (admin.nik=from_admin_nik) WHERE from_admin_nik= ? AND for_guest_nik=? OR from_admin_nik= ? AND for_guest_nik= ? ORDER BY room_created_date ASC;";
      $stmt = $link->prepare($query);
      $stmt->bindValue(1, $chatRoom->getAdmin());
